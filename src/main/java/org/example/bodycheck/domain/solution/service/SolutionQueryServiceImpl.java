...
public class SolutionQueryServiceImpl implements SolutionQueryService {
	...
	public List<Solution> getSolutionList(Long memberId, String exerciseType, Integer period, Integer page) {
		...
		if (page == 0) {
			...
			if (redisService.existKey("cursor:" + memberId)) {
				String cursorValue = redisService.getValues("cursor:" + memberId);
				cursor = LocalDateTime.parse(cursorValue);
			} else {
		...
		if (hasNext) {
			LocalDateTime newCursor = solutionList.get(pageSize - 1).getCreatedAt();
			redisService.saveKeyValueWithTtl("cursor:" + memberId, String.valueOf(newCursor), 1000 * 60 * 60);
		} else {
			LocalDateTime newCursor = solutionList.getLast().getCreatedAt();
			redisService.saveKeyValueWithTtl("cursor:" + memberId, String.valueOf(newCursor), 1000 * 60 * 60);
		}
	...
	private String getSolutionContent(Long solutionId) {
		Solution solution = solutionRepository.findById(solutionId)
			.orElseThrow(() -> new GeneralException(ErrorStatus.SOLUTION_NOT_FOUND));
