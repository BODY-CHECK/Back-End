...
public class AttendanceService {
	...
	@Transactional
	public AttendanceResponseDto.AttendanceCheckDto check(Member member, Long exerciseId,
		SolutionRequestDto.PromptDto request) {
		LocalDate today = LocalDate.now();
		DayOfWeek dayOfWeek = today.getDayOfWeek();

		int dayNumber = (dayOfWeek.getValue() % 7) + 1;

		List<Routine> routines = routineRepository.findByMember_IdAndWeekIdAndExercise_Id(member.getId(), dayNumber,
			exerciseId);
		...
		attendance = attendanceRepository.findByMemberAndDate(member, today)
			.orElseThrow(() -> new GeneralHandler(ErrorStatus.ATTENDANCE_NOT_FOUND));
	...
	public List<AttendanceResponseDto.AttendanceDto> getAttendance(Member member, String yearMonth) {
		...
		List<Attendance> attendanceList = attendanceRepository.findAllByMemberAndDateBetween(member, startOfMonth,
			endOfMonth);
