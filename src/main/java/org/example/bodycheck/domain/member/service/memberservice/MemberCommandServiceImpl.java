...
public class MemberCommandServiceImpl implements MemberCommandService {
	...
	public JwtTokenDto signIn(MemberRequestDto.SignInDto request) {
		...
		Member member = memberRepository.findByEmail(clientEmail)
			.orElseThrow(() -> new GeneralHandler(ErrorStatus.LOGIN_UNAUTHORIZED));
		...
		redisService.saveKeyValueWithTtl("refresh:" + clientEmail, jwtTokenDto.getRefreshToken(),
		...
		// 	refreshToken = refreshRepository.findByMember_Id(member.getId())
		// 		.orElseThrow(() -> new GeneralHandler(ErrorStatus.TOKEN_UNSUPPORTED));
	...
	public MemberResponseDto.SocialLoginResponseDto handleSocialLogin(String clientEmail, LoginType loginType) {
		...
		if (memberQueryService.isRegisteredWithEmail(member) || !memberQueryService.isRegisteredWithSocial(member,
			loginType)) {
	...
	public void logout(String clientEmail, String accessToken) {
		...
		// // 이전 로직 - 리프레시 토큰을 DB에 저장 할 경우
		// RefreshToken deleteRefreshToken = refreshRepository.findByMember_Id(memberId)
		// 	.orElseThrow(() -> new GeneralHandler(ErrorStatus.TOKEN_NOT_EXIST));
	...
	public JwtTokenDto refreshToken(MemberRequestDto.RefreshTokenDto request) {
		...
		if (!redisService.existKey("refresh:" + clientEmail) || !token.equals(
			redisService.getValues("refresh:" + clientEmail))) {
			throw new GeneralHandler(ErrorStatus.TOKEN_NOT_EXIST);
		}
		...
		// // 이전 로직 - 리프레시 토큰을 DB에 저장 할 경우
		// RefreshToken deleteRefreshToken = refreshRepository.findByRefreshToken(token)
		// 	.orElseThrow(() -> new GeneralHandler(ErrorStatus.TOKEN_NOT_EXIST));
		// refreshRepository.delete(deleteRefreshToken);
